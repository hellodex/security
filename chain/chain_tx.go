...
func HandleMessage(t *config.ChainConfig, messageStr string, to string, typecode string,
	...
	if wg.ChainCode == "SOLANA" {
		...
		if len(conf.LatestBlockHash) > 0 {
			...
			mylog.Infof("api传递GetLatestBlockhash %s ", conf.LatestBlockHash)
		} else {
			hashResult, err := GetLatestBlockhashFromMultipleClients(rpcList, rpc.CommitmentFinalized)
			// 计算耗时并记录。
			timeEnd := time.Now().UnixMilli() - timeStart
		...
		} else if conf.TxChannel != nil && conf.TxChannel.Type == 2 {
			mylog.Infof("Jito 通道提交, url=%s", conf.TxChannel.Url)
			txhash, err = SendTransactionJito(context.Background(), tx, conf.TxChannel)
			if err == nil {
				status = "confirmed"
			}
		} else {
